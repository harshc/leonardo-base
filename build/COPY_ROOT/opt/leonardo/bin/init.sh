...
function init_set_workspace() {
    ...
    if [[ $WORKSPACE_MOUNTED == "true" ]]; then
        test_file=${WORKSPACE}/.leonardo-permissions-test
        touch $test_file
...
function init_create_directories() {
  ...
  chown root.leonardo /run/http_ports
...
function init_source_preflight_script() {
    ...
    if [[ ! -f /opt/leonardo/bin/preflight.sh ]]; then
        printf "Not found\n"
    else
        source /opt/leonardo/bin/preflight.sh
    fi
}
...
function init_get_provisioning_script() {
    ...
    if [[ -n  $PROVISIONING_SCRIPT ]]; then
        file="/opt/leonardo/bin/provisioning.sh"
...
function init_debug_print() {
    if [[ -n $DEBUG ]]; then
        ...
        printf "\n--------------------------------------------\n"
        printf "/opt/leonardo/etc/environment.sh...\n\n"
        cat /opt/leonardo/etc/environment.sh
        printf "\n--------------------------------------------\n"
        printf ".bashrc...\n\n"
        cat /root/.bashrc
        printf "\n---------- END DEBUG INFO---------- \n\n\n"
